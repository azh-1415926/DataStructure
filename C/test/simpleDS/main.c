...
void test1(){
    ...
    for(int i=0;i<N;i++){
        seqListInsert(list,&array[i],i);
        if(*(int*)seqListIndexSearch(list,i)!=array[i]){
            printf("error,index of %d is %d,but get data:%d\n",i,array[i],*(int*)seqListIndexSearch(list,i));
            free(array);
            seqListFree(&list);
            return;
        }
    ...
    for(int i=0;i<N;i++){
        if(seqListDataSearch(list,&array[i],compare)!=i){
            printf("error,%d's index is %d,but get index:%d\n",i,array[i],seqListDataSearch(list,&array[i],compare));
            free(array);
            seqListFree(&list);
            return;
        }
    ...
    for(int i=0;i<2*N;i++){
        if(i<N){
            ...
            if(*(int*)seqListIndexSearch(temp2,i)!=*(int*)seqListIndexSearch(temp,i-N)){
                printf("error,index of %d is %d,but get data:%d\n",i,array[i-N],*(int*)seqListIndexSearch(temp2,i));
                free(array);
                seqListFree(&list);
                seqListFree(&temp);
                seqListFree(&temp2);
    ...
    for(int i=0;i<N;i++){
        if(*(int*)seqListIndexSearch(temp,i)!=*(int*)seqListIndexSearch(temp2,i)){
            printf("error,index of %d is %d,but get data:%d\n",i,array[i],*(int*)seqListIndexSearch(temp,i));
            free(array);
            seqListFree(&list);
            seqListFree(&temp);
            seqListFree(&temp2);
